...
//    open Chapter5.Math
    ...
    let reach_station (station:Station) (self:Ship) (s:PoliceChase) =
      co{
        do! yield_
        let dir = Vector2<_>.Normalize(station.Position - self.Position)
        if Vector2<_>.Distance(station.Position, self.Position) <= field_size * 1.0e-1 then
          let zero_velocity =
            co{
              do! yield_
    ...
    let patrol_ai (s:PoliceChase) =
      ...
      repeat_ ((healthy_and_fueled => attack self (s.Pirate)) .||>
               (need_docking       => reach_station s.PoliceStation self s))

    let demons_ai (s:PoliceChase) =
      let self = s.Demons
      let evil_and_powered =
        co{
          do! yield_
          return self.Integrity > self.MaxIntegrity * 0.1 && self.Fuel > self.MaxFuel * 0.1
        }
      let need_more_power = 
        co{
          do! yield_
          let! p = evil_and_powered
          return not p
        }
      repeat_ ((evil_and_powered => (attack (s.Demons) (s.Pirate))) .||>
               (need_more_power  => reach_station s.DemonPortal self s))
    ...
    let cargo_ai (s:PoliceChase) =
      ...
      co{
        do! yield_
        do! reach_station s.PoliceStation self s
      } |> repeat_
      ...
      let s =
        {
          ...
          DemonPortal = 
            { 
              Position = { X = field_size * 0.6; Y = field_size } * 0.75
              Integrity          = 100.0<_>
              MaxIntegrity       = 100.0<_>
            }
          Demons      =
            {
              Position        = { X = field_size; Y = field_size * 0.8 } * 0.65
              Velocity        = Vector2<_>.Zero
              DryMass         = 4.5e4<_>
              Fuel            = 2.2e6<_>
              MaxFuel         = 2.2e6<_>
    ...
    let simulation_step (s:PoliceChase) =
      ...
      if Vector2<_>.Distance(s.Demons.Position, s.Pirate.Position) < s.Demons.WeaponsRange then
        do s.Pirate.Integrity <- s.Pirate.Integrity - s.Demons.Damage * dt
      if Vector2<_>.Distance(s.Demons.Position, s.Cargo.Position) < s.Demons.WeaponsRange then
        do s.Pirate.Integrity <- s.Cargo.Integrity - s.Demons.Damage * dt
      if Vector2<_>.Distance(s.Demons.Position, s.Patrol.Position) < s.Demons.WeaponsRange then
        do s.Patrol.Integrity <- s.Patrol.Integrity - s.Demons.Damage * dt
    ...
    let print(s:PoliceChase) =
      ...
      do Console.Write("¤")
      do set_cursor_on_station (s.DemonPortal)
      do Console.Write("O")
      ...
      do set_cursor_on_ship (s.Demons)
      do Console.Write((ship_fuel s.Demons) + "S" + (ship_integrity s.Demons))
