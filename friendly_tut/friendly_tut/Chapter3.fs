...
module Chapter3
  module SmallAsteroidFieldSimulation =
      ...
      [
        for i = 1 to num_asteroids do
          ...
          *)
          ...
          let vx = max_velocity * (rand.NextDouble() * 2.0 - 1.0) * 0.8
          let vy = max_velocity * (rand.NextDouble() * 2.0 - 1.0) * 0.8
          (*
    ...
    *)
    ...
    let clamp_asteroids (Ap:Vector2<_>, Av, Bp, Bv) =
      if (Ap.X - Bp.X < 1.0e+4<m> && Ap.Y - Bp.Y < 1.0e+4<m>) ||
         (Bp.X - Ap.X < 1.0e+4<m> && Bp.Y - Ap.Y < 1.0e+4<m>) then
         Ap, { X = (Av.X + Bv.X)/2.0; Y = (Av.Y + Bv.Y)/2.0 }
      else Ap,Av
    (*
